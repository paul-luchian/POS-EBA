...
public class AdminServicesImpl implements AdminServices {
		...
		if(user_role.equals("ADMIN"))
		{
			...
			try {
				...
				if (profile_response.getStatusLine().getStatusCode() == 204) {
					...
					String url_token = "http://localhost:8080/POS-EBA-RSDB/server1/tokens/" + userId;
					HttpDelete request_token = new HttpDelete(url_token);
					token_response = client_profile.execute(request_token);
					if (token_response.getStatusLine().getStatusCode() == 200) {
	...
	public String getUserRole(String jwtToken)
	{
		...
		JSONObject token_json = new JSONObject(ret[1]);
		return token_json.get("role").toString();
	}
